...
# Pull all of the artist's albums (prof keeps album only)
...
results.to_csv('drake_songs_list.csv', index=True, index_label="Index")
print("Songs list saved → drake_songs_list.csv")
...
input_data_in_dataframe = pd.read_csv("drake_songs_clean.csv")
...
for song_title in cleanedsonglist:
    ...
    if (song is not None):
        if (song.lyrics is not None):
            # Delete the first line which often has metadata (prof example)
...
lyrics_results.to_csv('drake_lyrics.csv', index=True, index_label="Index")
print("Lyrics saved → drake_lyrics.csv")
